...
Zustands\"ubergangsdiagramm für die Befehlsausführung innerhalb der ALU
}
...
Anschlie"send werden auf Grundlage die zwei Operanden-Signale \Vhdl{s\_op1} und \Vhdl{s\_op2} mit einem Immediate vom Daten-Eingang belegt, oder es wird ein Register-Lesezugriff angesto"sen, dessen Ergebnis im nächsten Takt zur Verf\"ugung steht.
...
Ein Gro"steil der Operationen ist \"uber die \Vhdl{IEEE.NUMERIC\_STD.ALL} Operatoren realisiert und innerhalb dieses Taktes vollständig abgeschlossen.
...
Da der Standard-Operator \Vhdl{sar} nicht durch die gegebenen Entwicklungsumgebung synthetisierbar ist, wird bei einem arithmetischen Rechtsshift um {n} Stellen zusätzlich das oberste Bit des ersten Operanden zwischengespeichert, um im Nachfolgenden State – wenn n\"otig – die oberen {n} Bits auf {1} zu setzen.\vspace{10pt}
