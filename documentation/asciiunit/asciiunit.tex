...
\begin{figure}[H]
	\centering
		\includegraphics[width=0.1\textwidth]{Display.png}
	\caption{Beispiel für Textausgabe; jedes darstellbare Zeichen wird abgebildet}
...
\begin{figure}[H]
	...
	\caption{Veranschaulichung der Adressberechnung anhand des Pixelfelds: Links oben wird von 0 ab nach rechts zeilenweise hochgezählt. Die Adresse des Zeichens zu dem ein Pixel mit Position (x,y) gehört wird wie folgt berechnet: \begin{math} \lfloor \frac{x}{8} \rfloor + \lfloor \frac{y}{8} \rfloor * 64 \end{math}// Aus dem 64-Bitvektor berechnet man das Bit für das aktuelle Pixel so: \begin{math}(x mod 8) + (y mod 8) * 8 }\end{math}
	\label{fig:pixels}
\end{figure}
